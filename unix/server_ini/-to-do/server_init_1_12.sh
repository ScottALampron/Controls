...
if [ "${RSA_SSH_KEY_CREATION}" = true ]; then
    ...
    if [ "${RSA_BIT_KEY_CREATION}" = 4096 ]; then
        ...
        if [[ "${CUSTOM_RSA_PASSWD}" = true ]]; then
            ssh-keygen -f "${home_directory}"/.ssh/"${RSA_FILE_NAME}" -t rsa -b 4096 -N "${RSA_PASSWD}" -C "$HOSTNAME"@chaos
            echo "rsa pass = '${RSA_PASSWD}'"
        fi
        ## Using blank passwd
        if [[ "${CUSTOM_RSA_PASSWD}" = false ]]; then
            echo -e "\n\n\n\n" | ssh-keygen -f ~/.ssh/"${RSA_FILE_NAME}" -t rsa -b 4096 -C "$HOSTNAME"@chaos
    ...
    if [ "${RSA_BIT_KEY_CREATION}" != 4096 ]; then
        ## Using custom passwd
        if [[ "${CUSTOM_RSA_PASSWD}" = true ]]; then
            ssh-keygen -f "${home_directory}"/.ssh/"${RSA_FILE_NAME}" -t rsa -N "${RSA_PASSWD}" -C "$HOSTNAME"@chaos
            echo "rsa pass = '${RSA_PASSWD}'"
        fi
        ## Using blank passwd
        if [[ "${CUSTOM_RSA_PASSWD}" = false ]]; then
...
if [ "${DSA_SSH_KEY_CREATION}" = true ]; then
    ...
    if [[ "${CUSTOM_DSA_PASSWD}" = true ]]; then
        ssh-keygen -f "${home_directory}"/.ssh/"${DSA_FILE_NAME}" -t dsa -N "${DSA_PASSWD}" -C "$HOSTNAME"@chaos
        echo "dsa completed pass = '${DSA_PASSWD}'"
    fi

    ## Using blank passwd
    if [[ "${CUSTOM_DSA_PASSWD}" = false ]]; then
        echo -e "\n\n\n\n" | ssh-keygen -f ~/.ssh/"${DSA_FILE_NAME}" -t dsa -C "$HOSTNAME"@chaos
...
if [ "${ECDSA_SSH_KEY_CREATION}" = true ]; then
    ...
    if [ "${ECDSA_BIT_KEY_CREATION}" = 256 ]; then
        ...
        if [[ "${CUSTOM_ECDSA_PASSWD}" = true ]]; then
            ssh-keygen -f "${home_directory}"/.ssh/"${ECDSA_FILE_NAME}" -t ecdsa -b 256 -N "${ECDSA_PASSWD}" -C "$HOSTNAME"@chaos
            echo "ecdsa pass = '${ECDSA_PASSWD}'"
        fi
        ## Using blank passwd
        if [[ "${CUSTOM_ECDSA_PASSWD}" = false ]]; then
            echo -e "\n\n\n\n" | ssh-keygen -f ~/.ssh/"${ECDSA_FILE_NAME}" -t ecdsa -b 256 -C "$HOSTNAME"@chaos
    ...
    if [ "${ECDSA_BIT_KEY_CREATION}" = 384 ]; then
        ## Using custom passwd
        if [[ "${CUSTOM_ECDSA_PASSWD}" = true ]]; then
            ssh-keygen -f "${home_directory}"/.ssh/"${ECDSA_FILE_NAME}" -t ecdsa -b 384 -N "${ECDSA_PASSWD}" -C "$HOSTNAME"@chaos
            echo "ecdsa pass = '${ECDSA_PASSWD}'"
        fi
        ## Using blank passwd
        if [[ "${CUSTOM_ECDSA_PASSWD}" = false ]]; then
            echo -e "\n\n\n\n" | ssh-keygen -f ~/.ssh/"${ECDSA_FILE_NAME}" -t ecdsa -b 384 -C "$HOSTNAME"@chaos
            echo 'ecdsa completed no pass'
        fi
    fi
    ## 521 bit encryption
    if [ "${ECDSA_BIT_KEY_CREATION}" = 521 ]; then
        ## Using custom passwd
        if [[ "${CUSTOM_ECDSA_PASSWD}" = true ]]; then
            ssh-keygen -f "${home_directory}"/.ssh/"${ECDSA_FILE_NAME}" -t ecdsa -b 521 -N "${ECDSA_PASSWD}" -C "$HOSTNAME"@chaos
            echo "ecdsa pass = '${ECDSA_PASSWD}'"
        fi
        ## Using blank passwd
        if [[ "${CUSTOM_ECDSA_PASSWD}" = false ]]; then
            echo -e "\n\n\n\n" | ssh-keygen -f ~/.ssh/"${ECDSA_FILE_NAME}" -t ecdsa -b 521 -C "$HOSTNAME"@chaos
            echo 'ecdsa completed no pass'
        fi
    fi
fi
...
if [ "${ED25519_SSH_KEY_CREATION}" = true ]; then
    ...
    if [[ "${CUSTOM_ED25519_PASSWD}" = true ]]; then
        ssh-keygen -f "${home_directory}"/.ssh/"${ED25519_FILE_NAME}" -t ed25519 -N "${ED25519_PASSWD}" -C "$HOSTNAME"@chaos
